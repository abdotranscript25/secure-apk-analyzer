...
public class UltimateAnalyzer {
    ...
    public static UltimateReport analyze(String apkPath) throws Exception {
        ...
        try (net.dongliu.apk.parser.ApkFile apkParser =
                     new net.dongliu.apk.parser.ApkFile(new File(apkPath))) {
    ...
    private static void analyzeManifestFromString(String content, UltimateReport report) {
        ...
        // Permissions
        Pattern permPat = Pattern.compile(
                    ...
                    if (b >= 0x20 && b < 0x7F) {
                        ...
                        while (httpM.find()) {
                            ...
                            if (report.urlsFound.add(url)) {
                                report.tryAdd(report.networkFindings,
                                        new SecurityFinding(
                                                FindingType.INSECURE_CONNECTION,
                                                file.getName(),
                                                Collections.singletonList("HTTP_URL_DEX"),
                                                url, Severity.HIGH,
                        ...
                        while (emailM.find()) {
                            ...
                            if (!isWhitelistedEmail(email))
                                report.emailsFound.add(email);
                        }

                        // IP privées
                        Matcher ipM = IP_PRIVATE_PATTERN.matcher(s);
                        while (ipM.find())
                            report.ipAddresses.add(ipM.group());
